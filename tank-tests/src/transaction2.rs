...
use tank::{Connection, Entity, FixedDecimal, Transaction, expr};
...
pub async fn transaction2<C: Connection>(connection: &mut C) {
    ...
    let accounts = [
        Account {
            id: "A".into(),
            balance: Decimal::new(1000_00, 2).into(),
            active: true,
        ...
        Account {
            id: "C".into(),
            balance: Decimal::new(0_00, 2).into(),
            active: true,
            metadata: None,
            payload: None,
        },
    ...
    let amount = Decimal::new(200_00, 2);
    a.balance.0 -= amount;
    b.balance.0 += amount;
    ...
    assert_eq!(a_after_dec, Decimal::new(800_00, 2));
    assert_eq!(b_after_dec, Decimal::new(700_00, 2));

    // Transfer 300_00 B -> C and rollback
    let mut tx2 = connection
        .begin()
        .await
        .expect("Could not begin second transaction");
    ...
    let t2_amount = Decimal::new(300_00, 2);
    ...
    assert_eq!(b_after_rb_dec, Decimal::new(700_00, 2));
    assert_eq!(c_after_rb_dec, Decimal::new(0_00, 2));

    // Delete account C and commit
    let mut tx3 = connection
        .begin()
        .await
        .expect("Could not begin third transaction");
    ...
    c_entity
        .delete(&mut tx3)
        .await
        .expect("Could not delete C in tx3");
